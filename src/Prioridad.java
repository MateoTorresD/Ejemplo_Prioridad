...
public class Prioridad {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            System.out.print("Prioridad de " + nombre + " (menor número = mayor prioridad): ");
            int prioridad = sc.nextInt();

            procesos.add(new Proceso(nombre, rafaga, llegada, prioridad));
        }

        ejecutarPorPrioridad(procesos);
    }

    public static void ejecutarPorPrioridad(List<Proceso> listaOriginal) {
        List<Proceso> lista = new ArrayList<>(listaOriginal);
        List<Proceso> completados = new ArrayList<>();
        int tiempo = 0;

        while (completados.size() < lista.size()) {
            // Filtrar procesos que han llegado y no han sido completados
